...
if(platform.toString() === "windows")
{
    ...
    {
        await $`cmake --install builds/vs2022 --prefix Release --component VoicevoxTTS_VST3`;
        await $`cmake --install builds/vs2022 --prefix Release --component VoicevoxTTS_CLAP`;
    }
    else
    {
        if(flags.vst3)
...
else if(platform.toString() === "darwin")
{
    if(allPluginFormatFlagsAreFalse)
    {
        await $`cmake --install builds/xcode --prefix Release --component VoicevoxTTS_AU`;
        await $`cmake --install builds/xcode --prefix Release --component VoicevoxTTS_VST3`;
        await $`cmake --install builds/xcode --prefix Release --component VoicevoxTTS_CLAP`;
    }
    else
    {
        if(flags.au)
...
else if(platform.toString() === "linux")
{
    if(allPluginFormatFlagsAreFalse)
    {
        await $`cmake --install builds/ninja-single-release --prefix Release --component VoicevoxTTS_VST3`;
        await $`cmake --install builds/ninja-single-release --prefix Release --component VoicevoxTTS_CLAP`;
    }
    else
    {
        if(flags.vst3)
